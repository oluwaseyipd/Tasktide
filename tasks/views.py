...
class CompleteTaskListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Task.objects.filter(user=self.request.user, completed=True).order_by(
            "-created_at"
        )
...
def task_overview(request):
    ...
    # This was previously a template render; now just return context or adapt for API use
    return context
